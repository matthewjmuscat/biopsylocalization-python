...
- convex MC simulation now has typed patient contracts, containment/dose output
  collectors, and a singleton legacy adapter; the kernel-facing containment and
  dose loop bodies still need deeper extraction after parity checks,
...
- `MCReferenceKeys`, `MCContainmentSimulationConfig`, `MCDoseSimulationConfig`, `LegacyMCKeyBundle`, containment/dose output collectors, patient containment inventory/dilation-bank helpers, and `run_patient_mc_convex_legacy_adapter(...)` live in `python_files_dcm_meta_based/mc/simulation/per_patient/` as the additive patient-local MC simulation landing zone; `MC_simulator_convex.simulator_parallel(...)` remains untouched as the oracle.
...
- `python_files_dcm_meta_based/legacy_data_keys.py` owns generic legacy master-info, patient-reference, structure-record, structure-geometry, nested dataframe-store, and artifact sentinel spellings for additive adapters; older live mutation wrappers remain deferred until they cross a patient/runner/artifact boundary.
...
- `run_patient_target_dil_optimizer_v2_live_adapter(...)` lives in `python_files_dcm_meta_based/biopsy_optimizer/v2/per_patient/live_adapter.py` as a singleton-patient runner bridge around the current optimizer-v2 live integration surface.
...
| 23 | Optimizer v1 | `biopsy_optimizer.v1.per_patient.legacy_adapter.run_patient_optimizer_v1_legacy_adapter(...)`, `biopsy_optimizer_module_v1(...)` | Partial | Additive singleton-patient validation adapter exists. Keep it as a legacy-oracle bridge unless a clean v1 loop-body extraction becomes scientifically useful. |
| 24 | Optimizer v2 live integration | `biopsy_optimizer.v2.per_patient.live_adapter.run_patient_target_dil_optimizer_v2_live_adapter(...)`, `run_target_dil_optimizer_v2_for_live_simulated_family(...)` | Partial | Additive singleton-patient adapter exists around the current live integration surface. Keep it as the runner bridge until a deeper loop-body extraction is worth the extra validation cost. |
...
| 31 | MC simulation | `mc.simulation.per_patient.run_patient_mc_convex_legacy_adapter(...)`, `MC_simulator_convex.simulator_parallel(...)` | Partial | Typed MC simulation contracts, package-local legacy key contracts, containment/dose output collectors, patient containment inventory/dilation-bank helpers, and a singleton adapter now exist. Next extract the containment kernel-call loop and statistics writebacks into `mc/simulation/per_patient/containment.py`, then dose/dose-gradient bodies into `mc/simulation/per_patient/dose.py` after parity checks. |
...
2. Continue extracting the patient containment simulation body from
  `MC_simulator_convex.simulator_parallel(...)` into
  `mc/simulation/per_patient/containment.py`. The patient inventory and
  dilation-bank setup boundary exists; next move the kernel-call loop and
  containment-statistics writebacks behind the same contracts/output collectors.
3. Extract the patient dose and dose-gradient localization/DVH body from
  `MC_simulator_convex.simulator_parallel(...)` into
  `mc/simulation/per_patient/dose.py`, keeping the frozen oracle available for
  row/key parity checks.
