...
- `MCReferenceKeys`, `MCContainmentSimulationConfig`, `MCDoseSimulationConfig`, `LegacyMCKeyBundle`, containment/dose output collectors, a neutral patient relative-structure inventory module, containment biopsy/input/dilation-bank helpers, containment core wrappers around the existing helper/kernel APIs, and `run_patient_mc_convex_legacy_adapter(...)` live in `python_files_dcm_meta_based/mc/simulation/per_patient/` as the additive patient-local MC simulation landing zone; `MC_simulator_convex.simulator_parallel(...)` remains untouched as the oracle.
...
| 31 | MC simulation | `mc.simulation.per_patient.run_patient_mc_convex_legacy_adapter(...)`, `MC_simulator_convex.simulator_parallel(...)` | Partial | Typed MC simulation contracts, package-local legacy key contracts, containment/dose output collectors, a neutral relative-structure inventory module, containment setup/input builders, containment core wrappers around existing helper/kernel APIs, and a singleton adapter now exist. Next extract the per-biopsy containment statistics/writebacks, then dose/dose-gradient bodies into `mc/simulation/per_patient/dose.py` after parity checks. |
...
2. Continue extracting the patient containment simulation body from
  ...
  inventory, dilation-bank setup, per-biopsy/relative-structure input prep, and
  core helper/kernel API wrappers exist; next move the per-biopsy containment
  statistics/writebacks behind the same contracts/output collectors.
