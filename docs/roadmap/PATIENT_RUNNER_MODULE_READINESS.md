...
- `MCReferenceKeys`, `MCContainmentSimulationConfig`, `MCDoseSimulationConfig`, `LegacyMCKeyBundle`, containment/dose output collectors, and `run_patient_mc_convex_legacy_adapter(...)` live in `python_files_dcm_meta_based/mc/simulation/per_patient/` as the additive patient-local MC simulation landing zone; `MC_simulator_convex.simulator_parallel(...)` remains untouched as the oracle.
...
- `python_files_dcm_meta_based/legacy_data_keys.py` owns generic legacy master-info, patient-reference, structure-record, nested dataframe-store, and artifact sentinel spellings for additive adapters; older live mutation wrappers remain deferred until they cross a patient/runner/artifact boundary.
...
| 31 | MC simulation | `mc.simulation.per_patient.run_patient_mc_convex_legacy_adapter(...)`, `MC_simulator_convex.simulator_parallel(...)` | Partial | Typed MC simulation contracts, package-local legacy key contracts, containment/dose output collectors, and a singleton adapter now exist. Next extract the containment/statistics and dose/dose-gradient loop bodies into `mc/simulation/per_patient/containment.py` and `mc/simulation/per_patient/dose.py` after parity checks. |
...
2. Extract the patient containment simulation and containment-statistics body
  from `MC_simulator_convex.simulator_parallel(...)` into
  `mc/simulation/per_patient/containment.py`, using the new contracts/output
  collectors as the stable boundary.
