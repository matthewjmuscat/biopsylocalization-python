...
- convex MC simulation now has typed patient contracts, containment/dose output
  collectors, a singleton legacy adapter, containment helper slices, and additive
  dose/dose-gradient localization plus DVH helper slices; remaining MC work is
  deeper inactive/legacy dose statistics if needed and MR simulation,
