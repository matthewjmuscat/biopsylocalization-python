...
Long term, primary runtime outputs should favor storage-efficient,
machine-readable artifact classes. Human-readable dataframe/table products
should increasingly become post-run derived views, generated by explicit
constructors from manifests, array stores, provenance records, and table
contracts. This keeps runtime storage compact while still supporting notebooks,
CSV exports, manuscript analysis, QA review, and downstream statistical
pipelines when those views are actually needed.
...
on top of efficient primary artifacts.
