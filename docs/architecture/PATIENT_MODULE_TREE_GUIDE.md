...
| `python_files_dcm_meta_based/mc/simulation/` | Future MC simulation science | patient-local containment/dose/MR simulation setup, contracts, and loop-body extractions from the current simulator oracle files | MC prep transforms, patient-runner orchestration, GUI/startup policy, artifact assembly |
...
| MC convex simulation contracts/key registry/output collectors/singleton adapter | `python_files_dcm_meta_based/mc/simulation/per_patient/` |
