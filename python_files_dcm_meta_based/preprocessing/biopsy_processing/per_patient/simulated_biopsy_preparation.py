...
from preprocessing.biopsy_processing.simulated_biopsy_preparation import REMOVED_COHORT_DERIVED_SIMULATED_BIOPSY_LENGTH_METHODS
from preprocessing.biopsy_processing.simulated_biopsy_preparation import _build_real_biopsy_matches_by_dil
from preprocessing.biopsy_processing.simulated_biopsy_preparation import _find_nearest_dil_refnum
from preprocessing.biopsy_processing.simulated_biopsy_preparation import _find_real_biopsy_from_refnum
from preprocessing.biopsy_processing.simulated_biopsy_preparation import _find_structure_info_from_refnum
from preprocessing.biopsy_processing.simulated_biopsy_preparation import _format_simulated_identifier
from preprocessing.biopsy_processing.simulated_biopsy_preparation import _get_base_identifier_value
from preprocessing.biopsy_processing.simulated_biopsy_preparation import _get_simulated_biopsy_preparation_dict
from preprocessing.biopsy_processing.simulated_biopsy_preparation import _refresh_biopsy_info_dict
from preprocessing.biopsy_processing.simulated_biopsy_preparation import _renumber_biopsy_indices
from preprocessing.biopsy_processing.simulated_biopsy_preparation import _set_length_information
from preprocessing.biopsy_processing.simulated_biopsy_preparation import _set_multiplicity_information
from preprocessing.biopsy_processing.simulated_biopsy_preparation import _set_target_information
from preprocessing.biopsy_processing.simulated_biopsy_preparation import _update_preparation_complete
from preprocessing.biopsy_processing.simulated_biopsy_preparation import _validate_simulated_biopsy_length_method
...
def expand_patient_simulated_biopsy_multiplicity(*,
                                                 patient_uid,
                                                 pydicom_item,
                                                 bx_ref,
                                                 dil_ref,
                                                 master_structure_info_dict=None):
    real_biopsy_matches_by_dil = _build_real_biopsy_matches_by_dil(
        pydicom_item,
        bx_ref,
        dil_ref,
    )
    ...
    for simulated_group_key in simulated_biopsy_group_order:
        ...
        for multiplicity_index, specific_structure in enumerate(selected_structures, start=1):
            ...
            multiplicity_base_roi = _get_base_identifier_value(
                specific_structure,
                "ROI",
                "Multiplicity base ROI",
            )
            multiplicity_base_refnum = _get_base_identifier_value(
                specific_structure,
                "Ref #",
                "Multiplicity base ref #",
            )

            specific_structure["ROI"] = _format_simulated_identifier(
                multiplicity_base_roi,
                multiplicity_index,
            )
            specific_structure["Ref #"] = _format_simulated_identifier(
    ...
    return pydicom_item[bx_ref]


def determine_patient_simulated_biopsy_lengths(*,
                                               patient_uid,
                                               pydicom_item,
                                               bx_ref,
                                               dil_ref,
    ...
    real_bx_lengths_by_dil = defaultdict(list)
    ...
    for specific_structure in pydicom_item[bx_ref]:
        ...
        if target_structure_type == dil_ref and target_structure_refnum is not None:
            real_bx_lengths_by_dil[target_structure_refnum].append(float(contour_length_mm))
    ...
    for specific_structure in pydicom_item[bx_ref]:
        ...
        if simulated_biopsy_length_method == "full":
            ...
            if matched_real_biopsy_refnum is not None:
                matched_real_biopsy = _find_real_biopsy_from_refnum(
                    pydicom_item,
                    bx_ref,
                    matched_real_biopsy_refnum,
                )
            ...
            if matched_real_biopsy is not None and matched_real_biopsy.get("Reconstructed biopsy cylinder length (from contour data)") is not None:
                ...
                if relative_structure_type == dil_ref:
                    lengths_for_this_dil = real_bx_lengths_by_dil.get(relative_structure_refnum, [])
    ...
    return {
        ...
        "real_bx_lengths_by_dil": dict(real_bx_lengths_by_dil),
        "removed_cohort_derived_length_methods": REMOVED_COHORT_DERIVED_SIMULATED_BIOPSY_LENGTH_METHODS,
    }


def build_patient_simulated_biopsy_preparation_dataframe(*,
                                                         patient_uid,
                                                         pydicom_item,
                                                         bx_ref,
                                                         all_ref_key):
    ...
    patient_dataframe = pandas.DataFrame(patient_rows)
...
def prepare_patient_simulated_biopsies(*,
    ...
    length_results_dict = determine_patient_simulated_biopsy_lengths(
        patient_uid=patient_uid,
        pydicom_item=pydicom_item,
        bx_ref=bx_ref,
        dil_ref=dil_ref,
    ...
    return length_results_dict, patient_dataframe