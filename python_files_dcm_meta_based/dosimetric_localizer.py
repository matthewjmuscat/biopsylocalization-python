...
def dosimetric_localization_dataframe_version(bx_only_shifted_stacked_2darr,
                                              patientUID, 
                                              bx_structure_info_dict, 
                                              dose_lattice_phys_space_data_KDtree, 
                                              dose_vals_arr, 
                                              num_dose_calc_NN,
                                              num_MC_dose_sims,
                                              num_bx_points,
    ...
    The inverse distance weighting is specified by the idw_power, and the number of NN to find is determined by num_dose_calc_NN!
    """
    ...
    total_num_trials_and_nominal_times_bx_points = (num_MC_dose_sims+1)*num_bx_points

    # perfor NN search in one line!
    NN_search_output = dose_lattice_phys_space_data_KDtree.query(bx_only_shifted_stacked_2darr, k=num_dose_calc_NN)

    NN_distances_arr = NN_search_output[0] 
    NN_indices_arr = NN_search_output[1] 
    # Reshaping is performed to handle the case of num_dose_calc_NN = 1, if this is the output arrays are the wrong dimensions, the below two lines corrects this 
    NN_distances_2d_arr = np.reshape(NN_distances_arr, (total_num_trials_and_nominal_times_bx_points,num_dose_calc_NN)) # first index of the 2d arr is the point tested, second index is the kth nearest neighbour
    NN_indices_2d_arr = np.reshape(NN_indices_arr, (total_num_trials_and_nominal_times_bx_points,num_dose_calc_NN)) # first index of the 2d arr is the point tested, second index is the kth nearest neighbour
