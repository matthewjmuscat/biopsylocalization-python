...
EXPECTED_CURRENT_REGISTRY_COUNT = 67
...
OUTPUT_TABLE_SPECS = (
    ...
    _spec("cohort_bx_dvh_metrics_legacy", "Cohort: Bx DVH metrics", "Output CSVs/Cohort", "cohort", "dvh", "biopsy_metric", _key("Patient ID", "Bx index", "metric identity"), stitch_method="recompute_downstream", source_fragment_table_id="patient_dvh_metrics_legacy", validation_status="validated_phase3c_legacy", retention_policy="reimplement_later", downstream_usage="used_by_sibling_repo", notes="Legacy DVH metric surface; retain during validation, then compare against clean DVH service."),
    ...
    _spec("cohort_dil_global_tissue_scores_features", "Cohort: DIL global tissue scores and DIL features", "Output CSVs/Cohort", "cohort", "tissue_class", "biopsy_relative_structure", _key("Patient ID", "Bx index", "Relative structure type", "Relative structure index"), stitch_method="join_derived", validation_status="needs_aggregation_builder", downstream_usage="not_found_in_sibling_scan", notes="Derived convenience join of tissue scores and target-DIL radiomics; keep out of the core patient runner and regenerate downstream if needed."),
    ...
    _spec("cohort_tissue_volume_above_threshold", "Cohort: tissue volume above threshold", "Output CSVs/Cohort", "cohort", "tissue_class", "biopsy_threshold_structure", _key("Patient ID", "Bx index", "tissue/structure identity", "threshold identity"), stitch_method="concat_current_summary_fragments", source_fragment_table_id="biopsy_tissue_volume_above_threshold", validation_status="validated_phase3c", downstream_usage="not_found_in_sibling_scan"),
    ...
    _spec("patient_dvh_metrics_legacy", "DVH metrics", "Output CSVs/MC simulation", "patient", "dvh", "biopsy_metric", _key("Patient ID", "Bx index", "metric identity"), stitch_method="recompute_downstream", validation_status="phase3c_source_surface_legacy", retention_policy="reimplement_later", downstream_usage="used_by_sibling_repo"),
    ...
    _spec("biopsy_tissue_volume_above_threshold", "Tissue volume above threshold", "Output CSVs/MC simulation", "biopsy", "tissue_class", "biopsy_threshold_structure", _key("Patient ID", "Bx index", "tissue/structure identity", "threshold identity"), stitch_method="source_fragment", validation_status="phase3c_source_surface"),
