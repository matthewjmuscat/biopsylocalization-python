...
_MANIFEST_CONTRACTS: tuple[ManifestContract, ...] = (
    ...
    _contract(
        ...
        "Resolved scientific configuration snapshot",
        "run",
        "configuration_manifest",
        "current_durable",
        ("manifests/resolved_scientific_config.json",),
        "json",
        "config.snapshots.PIPELINE_CONFIG_SNAPSHOT_SCHEMA_VERSION",
        "config.snapshots.write_pipeline_config_snapshot",
        "Record the canonical scientific subset of PipelineConfig used by one run.",
        (
            "typed config class",
            "canonical scientific config payload",
            "scientific config SHA-256",
        ),
    ),
    _contract(
        "code_identity",
        "Effective source-tree identity",
        "run",
        "provenance_manifest",
        "current_durable",
        ("manifests/code_identity.json",),
        "json",
        "startup.code_identity.CODE_IDENTITY_SCHEMA_VERSION",
        "startup.code_identity.write_code_identity",
        "Record the Git revision and exact effective source state used by one run.",
        (
            "Git commit and branch",
            "dirty-state flag",
            "tracked-diff SHA-256",
            "non-ignored untracked-file SHA-256",
            "effective source-tree SHA-256",
        ),
    ),
    _contract(
        ...
        "provenance_manifest",
        "current_durable",
        ("manifests/runtime_environment_identity.json",),
        "json",
        "startup.runtime_environment.RUNTIME_ENVIRONMENT_IDENTITY_SCHEMA_VERSION",
        "startup.runtime_environment.write_runtime_environment_identity",
        "Record the interpreter, platform, installed packages, and dependency-lock identity used by one run.",
        (
            "Python version and implementation",
            "platform identity",
            "installed distribution-set SHA-256",
            "Pipfile.lock SHA-256",
            "combined runtime environment SHA-256",
        ),
    ),
    _contract(
        "run_compatibility_identity",
        "Strict run artifact compatibility identity",
        "run",
        "provenance_manifest",
        "current_durable",
        ("manifests/run_compatibility_identity.json",),
        "json",
        "output_artifacts.run_compatibility.RUN_COMPATIBILITY_IDENTITY_SCHEMA_VERSION",
        "output_artifacts.run_compatibility.write_run_compatibility_identity",
        "Fail-closed identity used before combining artifacts from separate runs.",
        (
            "scientific config SHA-256",
            "effective source-tree SHA-256",
            "input/bootstrap policy SHA-256",
            "runtime environment SHA-256",
            "output schema registry version",
            "combined compatibility identity SHA-256",
        ),
    ),
    ...
    *PATIENT_RUNNER_MANIFEST_CONTRACTS,
    _contract(
        ...
        (
            ...
            "source orchestration profile path and fingerprint",
            "scientific config snapshot path and fingerprint when available",
            "strict run compatibility identity for live workers",
            "requested jobs and execution mode",
            "failure policy",
            "worker job paths",
            "exact worker subprocess commands",
            "parent output directory",
        ),
        notes="Live scientific execution is currently limited to the anatomical_qa checkpoint.",
    ),
    _contract(
        "patient_worker_job",
        ...
        (
            ...
            "explicit DICOM role paths and patient input assignment fingerprint",
            "worker output directory",
            "job index",
            "run metadata",
            "scientific config snapshot reference",
            "strict run compatibility identity reference and fingerprint",
    ...
    _contract(
        ...
        "orchestration_manifest",
        "current_scaffold",
        ("worker_results/<job_id>_attempt_<n>.json",),
        "json",
