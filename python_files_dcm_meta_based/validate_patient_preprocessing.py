"""Compare a standalone checkpoint with a fresh independent legacy-input worker.
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--checkpoint-name", default="biopsy_preprocessing_shadow",
                        choices=("anatomical_qa", "biopsy_preprocessing_shadow", "optimization_shadow"))
    ...
    print(args.checkpoint_name + " comparison: " + ("PASS" if report["passed"] else "FAIL"))
    print(args.output_dir / (preprocessing_boundary(args.checkpoint_name).directory + "_pair_summary.json"))
