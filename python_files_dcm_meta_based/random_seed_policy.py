...
TRANSFORM_GENERATION_RESOLVED_PATIENT_SEEDS_KEY = "Transform generation resolved patient seeds"
...
def configure_runtime_random_seed_settings(master_structure_info_dict: MutableMapping[str, Any],
    ...
    random_info.setdefault(TRANSFORM_GENERATION_RESOLVED_PATIENT_SEEDS_KEY, {})
...
def resolve_transform_generation_patient_seed(transform_generation_random_seed: int | None,
                                             patient_uid: str) -> int | None:
    base_seed = _optional_int(transform_generation_random_seed)
    if base_seed is None:
        return None
    return _stable_derived_seed(base_seed, "transform_generation", "patient", patient_uid)
...
def build_transform_generation_patient_rng(master_structure_info_dict: MutableMapping[str, Any],
    ...
    base_seed = _optional_int(transform_generation_random_seed)
    if base_seed is None:
        base_seed = _optional_int(random_info.get(TRANSFORM_GENERATION_RANDOM_SEED_KEY))
    else:
        random_info[TRANSFORM_GENERATION_RANDOM_SEED_KEY] = base_seed

    resolved_seed = resolve_transform_generation_patient_seed(base_seed, str(patient_uid))
    metadata = {
        "random_seed_policy_schema_version": RANDOM_SEED_POLICY_SCHEMA_VERSION,
        "transform_generation_random_seed_base": base_seed,
        "transform_generation_random_seed_scope": TRANSFORM_GENERATION_SEED_SCOPE,
        "transform_generation_resolved_patient_seed": resolved_seed,
        "transform_generation_seeded": resolved_seed is not None,
    }
...
def runtime_random_seed_manifest_metadata(master_structure_info_dict: Mapping[str, Any]) -> dict[str, Any]:
    ...
    transform_generation_resolved_patient_seeds = random_info.get(TRANSFORM_GENERATION_RESOLVED_PATIENT_SEEDS_KEY, {})
    return {
        ...
        "transform_generation_resolved_patient_seeds": dict(transform_generation_resolved_patient_seeds),
