...
def MC_simulator_all_structs_dilations_generator_cupy(pydicom_item, structs_referenced_list, num_generated_transform_samples, rng=None):
    rng = _get_rng(rng)
    ...
    for structure_type in structs_referenced_list:
        for specific_structure_index, specific_structure in enumerate(pydicom_item[structure_type]):
            ...
            rng.normal(loc=sp_struct_uncertainty_data_dilations_mean_arr[0], scale=sp_struct_uncertainty_data_dilations_sigma_arr[0], size=num_generated_transform_samples),    
            rng.normal(loc=sp_struct_uncertainty_data_dilations_mean_arr[1], scale=sp_struct_uncertainty_data_dilations_sigma_arr[1], size=num_generated_transform_samples)],   
            dtype = float).T
...
def MC_simulator_all_structs_rotations_generator_cupy(pydicom_item, structs_referenced_list, num_generated_transform_samples, rng=None):
    rng = _get_rng(rng)
    ...
    for structure_type in structs_referenced_list:
        for specific_structure_index, specific_structure in enumerate(pydicom_item[structure_type]):
            ...
            rng.normal(loc=sp_struct_uncertainty_data_rotations_mean_arr[0], scale=sp_struct_uncertainty_data_rotations_sigma_arr[0], size=num_generated_transform_samples),  
            rng.normal(loc=sp_struct_uncertainty_data_rotations_mean_arr[1], scale=sp_struct_uncertainty_data_rotations_sigma_arr[1], size=num_generated_transform_samples),  
            rng.normal(loc=sp_struct_uncertainty_data_rotations_mean_arr[2], scale=sp_struct_uncertainty_data_rotations_sigma_arr[2], size=num_generated_transform_samples)],   
            dtype = float).T
...
def MC_simulator_shift_biopsy_structures_uniform_generator_cupy(patient_dict, bx_ref, biopsy_needle_compartment_length, num_simulations, rng=None):
    rng = _get_rng(rng)
    ...
    for specific_bx_structure_index, specific_bx_structure in enumerate(patient_dict[bx_ref]):
        ...
        structure_uniform_dist_shift_samples_arr = rng.uniform(low=0, high=core_length_compartment_length_difference, size=num_simulations)
...
def MC_simulator_shift_all_structures_generator_cupy(patient_dict, structs_referenced_list, num_generated_transform_samples, rng=None):
    rng = _get_rng(rng)
    ...
    for structure_type in structs_referenced_list:
        for specific_structure_index, specific_structure in enumerate(patient_dict[structure_type]):
            ...
            rng.normal(loc=sp_struct_uncertainty_data_mean_arr[0], scale=sp_struct_uncertainty_data_sigma_arr[0], size=num_generated_transform_samples),  
            rng.normal(loc=sp_struct_uncertainty_data_mean_arr[1], scale=sp_struct_uncertainty_data_sigma_arr[1], size=num_generated_transform_samples),  
            rng.normal(loc=sp_struct_uncertainty_data_mean_arr[2], scale=sp_struct_uncertainty_data_sigma_arr[2], size=num_generated_transform_samples)],   
            dtype = float).T
