...
def main():
    ...
    disp_figs = ques_funcs.ask_ok('Do you want to open all figure now?')
            ...
            for structs in structs_referenced_list:
                for specific_structure in pydicom_item[structs]:
                    ...
                    if plot_fig_bool == True:
                        ...
                        threeDdata_list_and_color.append('r')
                        ...
                        info["Patient Name"] = pydicom_item["Patient Name"]
                        info["Patient ID"] = pydicom_item["Patient ID"]
                        specific_structure_fig = plotting_funcs.arb_threeD_scatter_plotter(threeDdata_list_and_color,structure_centroids_list_and_color,centroid_line_sample_list_and_color,drawn_biopsy_list_and_color,**info)
                        specific_structure_fig = plotting_funcs.add_line(specific_structure_fig,centroid_line)
...
def structure_referencer(structure_dcm_dict, OAR_list,DIL_list,Bx_list):
    ...
    for UID, structure_item in structure_dcm_dict.items():
        bpsy_ref = [{"ROI":x.ROIName, "Ref #":x.ROINumber, "Raw contour pts": None, "Structure centroid pts": None, "Best fit line of centroid pts": None, "Centroid line sample pts": None, "Reconstructed structure pts": None, "Nearest neighbour pts": None, "Plot attributes": plot_attributes()} for x in structure_item.StructureSetROISequence if any(i in x.ROIName for i in Bx_list)]    
        OAR_ref = [{"ROI":x.ROIName, "Ref #":x.ROINumber, "Raw contour pts": None, "Structure centroid pts": None, "Best fit line of centroid pts": None, "Centroid line sample pts": None, "Reconstructed structure pts": None, "Nearest neighbour pts": None, "Plot attributes": plot_attributes()} for x in structure_item.StructureSetROISequence if any(i in x.ROIName for i in OAR_list)]
        DIL_ref = [{"ROI":x.ROIName, "Ref #":x.ROINumber, "Raw contour pts": None, "Structure centroid pts": None, "Best fit line of centroid pts": None, "Centroid line sample pts": None, "Reconstructed structure pts": None, "Nearest neighbour pts": None, "Plot attributes": plot_attributes()} for x in structure_item.StructureSetROISequence if any(i in x.ROIName for i in DIL_list)]
...
class plot_attributes:
    def __init__(self,plot_bool_init = False):
