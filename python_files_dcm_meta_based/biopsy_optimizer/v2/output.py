...
def build_target_dil_optimization_summary_dataframe(
    ...
    summary_row = {
        ...
        "Target optimizer final stage round index": winner_row.get("Stage round index", np.nan),
        "Target optimizer final stage appended trial block size": winner_row.get(
        ...
        "Target optimizer final stage capacity-packed cumulative trial prefix target": winner_row.get(
            "Stage capacity-packed cumulative trial prefix target",
            np.nan,
        ),
        "Target optimizer final stage max test structures per call budget": winner_row.get(
            "Stage max test structures per call budget",
            np.nan,
        ),
