"""Candidate-pool generation and visualization for optimizer v2."""
...
from biopsy_optimizer.v2.contracts import OptimizerV2CandidatePool
...
def build_target_candidate_lattice(
    ...
    """Build a fixed lattice that spans the target point cloud bounds."""
...
def prune_candidate_lattice_to_target_interior(
    ...
    """Keep only lattice points that lie inside the target structure interior."""
...
def build_target_candidate_pool(
    ...
    """Build the optimizer-v2 candidate pool from a target structure."""
...
def visualize_target_candidate_pool(
    ...
    """Render the candidate-pool debug views requested by the selector config."""
    ...
    if resolved_visualization_config.plot_candidate_lattice_bool:
        full_lattice_point_cloud = point_containment_tools.create_point_cloud(
            candidate_pool.full_lattice_points,
            np.array([0.0, 0.0, 0.0]),
        )
        candidate_point_cloud = point_containment_tools.create_point_cloud(
            candidate_pool.candidate_points,
            np.array([0.0, 1.0, 0.0]),
        )
        plotting_funcs.plot_geometries(
            full_lattice_point_cloud,
            candidate_point_cloud,
            target_point_cloud,
            *additional_clouds_list,
            label="Unknown",
        )
    ...
    if resolved_visualization_config.plot_selected_candidate_points_bool:
        ...
        selected_candidate_point_cloud = point_containment_tools.create_point_cloud(
            selected_candidate_points,
            np.array([1.0, 0.0, 1.0]),
        )
        plotting_funcs.plot_geometries(
            candidate_point_cloud,
            selected_candidate_point_cloud,
            target_point_cloud,
            *additional_clouds_list,
            label="Unknown",
        )
...
def _validate_xyz_points_array(points_array: np.ndarray, array_name: str) -> np.ndarray:
    """Validate a dense XYZ point array used by candidate-pool code."""
...
def _validate_zslices_list(target_zslices_list: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
    """Validate a list of constant-z polygon slices used as a target structure."""
    ...
    for zslice_index, zslice_points in enumerate(target_zslices_list):
        normalized_zslice_points = _validate_xyz_points_array(
            zslice_points,
            "target_zslices_list[{}]".format(zslice_index),
        )
