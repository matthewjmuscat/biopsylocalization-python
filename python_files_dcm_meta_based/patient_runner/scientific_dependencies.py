...
DEFAULT_PATIENT_SCIENTIFIC_STAGE_DEPENDENCIES = (
    ...
    PatientScientificStageDependency(
        ...
        summary="Runs optimizer stages against prepared anatomy and biopsy state.",
    ),
    PatientScientificStageDependency(
        stage_name=PatientStageName.MC_PREP,
        required_stage_names=(PatientStageName.PREPROCESSING,),
        summary="Builds MC transform state from prepared biopsy and anatomical structures.",
    ),
    PatientScientificStageDependency(
...
def patient_scientific_pathway_stage_names(
    ...
    """Return the full stage-node slice for a named scientific pathway."""
...
def validate_patient_scientific_stage_dependencies(
    ...
    *,
    ...
    """Validate that requested scientific stages obey dependency order.
    ...
    for stage_name in resolved_stage_names:
        dependency = _DEPENDENCIES_BY_STAGE_NAME.get(stage_name)
...
def validate_patient_scientific_pathway_dependencies(
    ...
    *,
    ...
    """Validate and return the full graph slice for a named pathway."""
...
def summarize_patient_scientific_dependency_graph() -> tuple[dict[str, Any], ...]:
    """Return JSON-ready summaries of the scientific stage dependency graph."""
    return tuple(
        {
        ...
        for dependency in DEFAULT_PATIENT_SCIENTIFIC_STAGE_DEPENDENCIES
    )
